...
namespace Interview.Web.Repository
{
    public class ProductRepository : IProductRepository
    {
        ...
        public Task<List<Product>> SearchProduct(Product product)
        {
            ...
            var query = new StringBuilder($"select P.Name,p.Description,P.ProductImageUris,p.ValidSkus from Instances.Products p inner join Instances.ProductCategories pc on  pc.InstanceId=p.InstanceId inner join Instances.Categories c on c.InstanceId=pc.CategoryInstanceId where ");
        ...
        {
            var productQuery = $"insert into Instances.Products values (@Name,@Description,@ProductImageUris,@ValidSkus,@CreatedTimestamp)";

            var categoryQuery = $"insert into Instances.Categories values (@CategoryName,@Description,@CreatedTimestamp)";
