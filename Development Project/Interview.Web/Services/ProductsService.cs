...
namespace Interview.Web.Services
{
	...
	public class ProductsService : IProductSerivce
	{
		...
		{
			Logger = logger ?? throw new ParameterRequiredException($"{nameof(ILogger<ProductsService>)}", "DI cannot be null");
			ProductRepo = productRepo ?? throw new ParameterRequiredException($"{nameof(IProductsRepo)}", "DI cannot be null");
		}
